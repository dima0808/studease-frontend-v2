...
const CollectionCard = (props) => {
  const { id, index, name, questionsCount, wide, selectedItems, usedInTests } = props;
  return (
    <ItemCard
      id={id}
      index={index}
      ...
      extraContent={
        <>
          <Info title="Questions count" description={questionsCount} icon={CountIcon} />
          <Info title="Used in the tests" description={usedInTests} icon={UsedIcon} />
        </>
      }
      actions={
        <>
          <Status isActive={usedInTests > 0} params={["In use", 'Not use']}/>
          <CardActions isSelected={selectedItems.some(i => i.id === id)} name={name} wide={wide} id={id} />
        </>
      }
    />
  );
